...
bool8 is_power_of_two(uintptr_t value)
{
...
{
	uintptr_t pointer, alignment_size, mod;

	ARENA_ASSERT(is_power_of_two(align));

	pointer = ptr;
	alignment_size = align;

	//bit wise 1000 - 1110
	mod = pointer & (alignment_size - 1);

	if (mod != 0)
	{
		pointer += alignment_size - mod;
	}
...
{
	//this means buf is pointer to a memory 
	//from that point how many bytes forward
	uintptr_t current_offset_position = (uintptr_t)arena->buff + (uintptr_t)arena->current_offset;	
	uintptr_t new_offset_start = align_forward(current_offset_position, alignment);
	//address space to offset number
	//buff = 0x1003
	// current_offset = 5
	// new_ptr_position = 0x1008
	//offset = 0x1008 - 0x1003 = 5
